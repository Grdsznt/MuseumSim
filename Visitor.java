...
{
    //animation mollyRun of visitor
    //molly run animation all direction, 6 mollyRun each
    ...
    //molly idle animation 4 direction, 6 mollyRun each
    ...
    //molly idle hoping 4 directoins, 6 mollyRun each
    ...
    //molly reading a book animation, only down direction, 18 mollyRun
    ...
    //molly checking phone animation, only down direction, 9 mollyRun
    ...
    //adam runing animation, 4 direction, 6 mollyRun each
    ...
    //adam idle animation, 4 direction and 6 mollyRun each
    ...
    //adam reading book animation, only down direction, 18 mollyRun
    ...
    //adam checking phone animation, only down direction, 9 mollyRun
    ...
    //instance variables
    private boolean mollyOrAdam;//true is molly, adam is false  
    private boolean willLookPhone, willReadBook, isMoving;//yes is true, no is false
    ...
    public Visitor(int time,double speed){
        this.speed = speed;
        visitDuration = time;
        numberOfVisitors++;
        mollyOrAdam = Greenfoot.getRandomNumber(2) == 0;//molly or adam is random
        mollyOrAdam = false;//for testing, remove after
        actNum = 0; frameNum = 0;
        willLookPhone = false; willReadBook = false; isMoving = false; 
        direction = 4;
        //set initial image
        if(mollyOrAdam){
    ...
    public void act()
    {
        //testing section, see if things go properly
        ...
        //animation section, set mollyOrAdam to true for molly and false for adam
        if(mollyOrAdam){
        ...
        //visitor leaves when they have enough of the museum
        if(visitDuration <= 0){
            numberOfVisitors--;
    ...
    //making visitor look at this phone
    public void lookAtPhone(){
        //cant look at phone if its already looking at phone
        if(!willLookPhone && !willReadBook){
            willLookPhone = true;
            frameNum = 0;//reset frames
        }
    }
    //make visitor read a book
    public void readBook(){
        //cant read book if its already reading book
        if(!willLookPhone && !willReadBook){
            willReadBook = true;
            frameNum = 0; //reset frames
        }
